...
CourseTable find_registered_courses(CourseTable x,GradeTable y,CourseTable valid_courses);
void show_output(CourseTable valid_courses);
...
int main(int argc,char* argv[])
{
    ...
    CourseTable taken_courses=find_registered_courses(courses,grades,sorted);
    show_output(taken_courses);
}
...
GradeTable read_grades(string address) 
{
	...
	else 
    {
        ...
        {
			temp = new Grade;
			temp->ID = string_to_int(ID);
			getline(file,grade);
			temp->grade = string_to_float(grade);
...
CourseTable find_registered_courses(CourseTable courses,GradeTable grades,CourseTable valid_courses)
{   
    int number_of_units=0;
    CourseTable taken_courses;
    int max;
    int valid;
    if(is_the_student_top(calculate_gpa(courses,grades)))
