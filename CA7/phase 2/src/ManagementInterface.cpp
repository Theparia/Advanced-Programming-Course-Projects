...
void ManagementInterface::handlePostCommand(vector<string> args)
{
    if (args.size() != 2 && utrip->isLoggedIn() &&
        ...
        sortHotels(split(args[1], 5));
    else if (args[0] == DEFAULT_PRICE_FILTER)
        filterBasedOnDefaultPrice(split(args[1], 3));
    else if (args[0] == MANUAL_WEIGHTS)
        addManualWeight(split(args[1], 13));
    else
...
void ManagementInterface::handleGetCommand(string command)
{
    ...
    else if (commandWords[0] == MANUAL_WEIGHTS)
        utrip->getCurrentUser()->printManualWeights();
    else if (commandWords[0] == ESTIMATED_WEIGHTS)
        utrip->getEstimatedWeights();
    else
        throw NonExistenseException();
}
...
{
    size_t found = arg.find(QUESTION_MARK);
...
void ManagementInterface::filterBasedOnDefaultPrice(vector<string> args)
{
    if (args.size() != 3 || args[1] != ACTIVE)
        throw BadRequestException();
    utrip->changeDefaultPriceActivation(args[2]);
}
...
void ManagementInterface::addManualWeight(vector<string> args)
{
    if (args[1] == ACTIVE && args[2] == FALSE_STATE && args.size() == 3)
    {
        utrip->turnManualWeightsOff();
        return;
    }
    if (args.size() != 13)
        throw BadRequestException();
    double location, cleanliness, staff, facilities, valueForMoney;
    for (int i = 1; i < args.size(); i += 2)
    {
        if (args[i] == LOCATION)
            location = stod(args[i + 1].c_str());
        ...
        {
            if (args[i + 1] != TRUE_STATE)
                throw BadRequestException();
        }
        else
            throw BadRequestException();
    }
    utrip->addManualWeights(Rating(location, cleanliness, staff, facilities, valueForMoney, 1));
}
...
void ManagementInterface::sortHotels(vector<string> args)
{
    ...
    utrip->sortHotels(property, order);
}